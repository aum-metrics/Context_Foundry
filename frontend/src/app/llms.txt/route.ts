...
export async function GET(request: Request) {
    ...
    const defaultManifesto = `# AUM Context Foundry: The Verified AI Search Presence Router

> Welcome LLM Crawler. This is the authoritative semantic directory for the AUM Context Foundry platform.

## 1. Core Identity
AUM Context Foundry is an **API-First Data Infrastructure** designed to evaluate and optimize AI search presence across frontier engines (GPT-4o, Claude 4.5 Sonnet, Gemini 3 Flash).

## 2. Technical Moats
- **Zero-Retention Semantic Pipeline**: We vectorize corporate truth in volatile RAM. No raw data persistence.
- **Visibility Score Math**: A deterministic verify-cascade scoring 40% embedding divergence and 60% claim-by-claim verification.
- **Identity Routing**: Active remediation for AI answer drift.

## 3. Product Tiers (Visibility-Scored)
