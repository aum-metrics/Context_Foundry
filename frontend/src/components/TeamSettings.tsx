...
export default function TeamSettings() {
    ...
    const handleInvite = async (e: React.FormEvent) => {
        ...
        const seatCap = organization.subscriptionTier === 'enterprise' ? 100 : organization.subscriptionTier === 'scale' ? 25 : organization.subscriptionTier === 'growth' ? 5 : 1;
        if (organization.activeSeats >= seatCap) {
            setError(`You have reached the maximum cap of ${seatCap} seats for the current plan.`);
            return;
        }
            ...
            <header className="mb-10 pb-6 border-b border-slate-200 dark:border-white/5 flex justify-between items-end">
                ...
                <div className="text-right">
                    ...
                    <div className="flex items-baseline space-x-1">
                        ...
                        <span className="text-slate-500 text-lg">/ {organization.subscriptionTier === 'enterprise' ? 100 : organization.subscriptionTier === 'scale' ? 25 : organization.subscriptionTier === 'growth' ? 5 : 1}</span>
                    </div>
                </div>
            </header>
                    ...
                    <div className="rounded-2xl border border-slate-200 dark:border-white/5 bg-white/50 dark:bg-slate-900/50 backdrop-blur-xl p-6 shadow-xl dark:shadow-none sticky top-6">
                        ...
                        <form onSubmit={handleInvite}>
                            <div className="space-y-4">
                                ...
                                <button
                                    type="submit"
                                    disabled={inviting || inviteSuccess || organization.activeSeats >= (organization.subscriptionTier === 'enterprise' ? 100 : organization.subscriptionTier === 'scale' ? 25 : organization.subscriptionTier === 'growth' ? 5 : 1)}
