...
Version: 2.1.4 (FINAL CRITICAL SUPABASE FIX)
Tagline: The Sound of Data Understanding

CRITICAL FIXES:
- FINAL FIX: PGRST100 Filter Error resolved by strictly enforcing the use of 
  the validated user_id_str (clean string UUID) across all functions.
- Corrected session initialization logic to prevent database calls on unauthenticated reruns.
"""
...
# Supabase Imports (Assuming necessary packages are installed via requirements.txt)
try:
...
# Import AUM Engine (Requires aum_engine.py file to be present)
try:
    from aum_engine import (
        AUMEngine, DomainIntelligence
    )
    # NOTE: Assuming SemanticJoinEngine, PromptInterpreter are accessible via AUMEngine's methods/init
...
# ============================================================================
# SUPABASE HELPERS & DB OPS (CRITICALLY FIXED)
# ============================================================================
...
def log_usage(supabase: Client, user_id: str, prompt: str, domain: str, result_rows: int = 0, cost: float = 0):
    ...
    def _operation():
        ...
        if not getattr(response, "data", None): st.warning("⚠️ Insert returned no data — check Supabase logs.")
        return response
...
def show_login_page(supabase: Client):
    ...
    st.markdown('<p class="tagline">Augmented Universal Metrics — The Sound of Data Understanding</p>', unsafe_allow_html=True)
    ...
    with col2:
        ...
        with tab2:
            with st.form("register_form"):
                ...
                if register_btn:
                    if not reg_email or not reg_password: st.error("Please fill all fields")
                    elif reg_password != reg_confirm: st.error("Passwords don't match")
                    elif len(reg_password) < 6: st.error("Password must be at least 6 characters")
                    else:
                        try:
                            response = supabase.auth.sign_up({"email": reg_email, "password": reg_password})
                            if response.user: st.success("✅ Account created! Please login.")
                        except Exception as e:
...
def render_main_ui(supabase: Client, user):
    ...
    with c2: 
        if 'project_id' in st.session_state: st.markdown(f"**📁 Project:** `{st.session_state.project_id[:8]}`")
    with c3:
    ...
    with st.sidebar:
        ...
        if uploaded_files:
            ...
            else: st.error("❌ Maximum 5 files allowed")
        ...
        if paid: st.success("✅ **Unlimited Access**")
        else:
    ...
    if st.session_state.get('query_count', 0) >= FREE_QUERY_LIMIT and not st.session_state.get('paid_access', False):
        ...
        if st.button("💳 Upgrade to Pro", type="primary"): show_payment_modal(supabase, user_id_str)
        return
...
def initialize_engine(supabase: Client, user_id: str, domain: str, model: str):
    ...
    try:
        ...
        if 'project_id' not in st.session_state: st.session_state.project_id = hashlib.md5(str(datetime.now()).encode()).hexdigest()[:8]
        st.success(f"✅ Engine ready! {len(engine.dataframes)} datasets loaded, {len(suggestions)} joins detected")
        
    except Exception as e: st.error(f"❌ Initialization failed: {str(e)}")
...
def render_join_configuration(supabase: Client, user_id: str):
    ...
    edited_df = st.data_editor(join_df, column_config={"confidence": st.column_config.ProgressColumn("Confidence", min_value=0, max_value=1)}, hide_index=True, use_container_width=True)
...
def render_visualizations():
    ...
    try:
        ...
        if task == 'rank' and dimensions and metrics:
            fig = px.bar(df.head(20), x=dimensions[0], y=metrics[0], title=f"{metrics[0]} by {dimensions[0]}", template='plotly_white'); fig.update_layout(height=500); st.plotly_chart(fig, use_container_width=True)
        elif task == 'trend' and query.get('time_column') and metrics:
            fig = px.line(df, x=query['time_column'], y=metrics[0], title=f"{metrics[0]} Trend", markers=True, template='plotly_white'); fig.update_layout(height=500); st.plotly_chart(fig, use_container_width=True)
            ...
            fig = px.imshow(pivot_numeric, title=f"{metrics[0]} Heatmap", aspect='auto', template='plotly_white'); fig.update_layout(height=500); st.plotly_chart(fig, use_container_width=True)
        else:
            numeric_cols = df.select_dtypes(include=[np.number]).columns[:2]
            if len(numeric_cols) >= 2:
                fig = px.scatter(df, x=numeric_cols[0], y=numeric_cols[1], title="Data Distribution", template='plotly_white'); fig.update_layout(height=500); st.plotly_chart(fig, use_container_width=True)
...
def main():
    ...
    if not user_id_str:
        st.error("Authentication session corrupted. Please log out and try again."); return
    ...
    if 'query_count' not in st.session_state or 'paid_access' not in st.session_state:
